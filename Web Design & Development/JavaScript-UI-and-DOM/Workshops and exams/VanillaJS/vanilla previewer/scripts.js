function createImagesPreviewer(selector, items) {
  ...
  imgsList.addEventListener('click', function (el) {
    if (el.target.parentNode.tagName == 'LI') {

      var title = el.target.parentNode.querySelector('span').innerText;
      var image = el.target.parentNode.querySelector('img').src;

      bigImage.src = image;
      bigImage.previousElementSibling.innerText = title;

    };
  });

  imgsList.addEventListener('mouseover', function (el) {
    if (el.target.parentNode.tagName == 'LI') {
      el.target.parentNode.style.background = 'grey';
      el.target.parentNode.style.borderRadius = '5px';
      el.target.parentNode.style.padding = '5px';
    };
  });

  imgsList.addEventListener('mouseout', function (el) {
    if (el.target.parentNode.tagName == 'LI') {
      el.target.parentNode.style.background = '';
    };
  });

  filter.addEventListener('input', function (el) {
    var pattern = el.target.value.toLowerCase();
    var imgCollection = imgsList.getElementsByTagName('span');

    for (var i = 0; i < imgCollection.length; i++) {
      var imgTitle = imgCollection[i].innerHTML.toLowerCase();
      if (imgTitle.indexOf(pattern) === -1) {
        imgCollection[i].parentElement.style.display = 'none';
      }
      else {
        imgCollection[i].parentElement.style.display = 'block';
      }
    }
  });
